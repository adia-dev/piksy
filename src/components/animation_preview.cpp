...
void AnimationPreview::adjust_pan_and_zoom_to_frame(int frame_index) {
    ...
    float desired_scale = 4.0f;  // You can make this configurable
...
void AnimationPreview::render_frame_thumbnails() {
    ...
    for (int i = 0; i < frame_count; ++i) {
        ...
        // Compute aspect ratio and display size
        ...
        // Context menu for deleting a frame
        ...
        if (ImGui::BeginDragDropSource()) {
            ...
            // Show a preview of the frame being dragged
...
void AnimationPreview::render_frame_list() {
    ...
    if (ImGui::BeginTable("FrameListTable", 2, ImGuiTableFlags_Resizable)) {
        // Set up columns: 75% for frame list, 25% for frame preview
        ...
        if (_state.animation_state.current_frame >= 0 &&
            ...
            if (texture) {
                // Compute aspect ratio and display size
                float aspect_ratio = static_cast<float>(frame.w) / frame.h;
                float preview_size = ImGui::GetContentRegionAvail().x;  // Maximize preview width
                ImVec2 display_size;
